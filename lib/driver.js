...
Driver.prototype.digitalWrite = function(value, callback) {
  ...
  Cylon.Logger.debug(`Writed value ${value} into pin ${this.pin}`);
  Cylon.Logger.debug(GPIO.print());
  callback(null, value);
}
...
Driver.prototype.digitalRead = function(callback) {
  ...
  Cylon.Logger.debug(`Read value ${value} from pin ${this.pin}`);
  Cylon.Logger.debug(GPIO.print())
  callback(null, value);
}
...
Driver.prototype.servoWrite = function(angle, callback) {
  ...
  Cylon.Logger.debug(`Moved servo with angle ${angle}`)
  Cylon.Logger.debug(GPIO.print())
  callback(null, angle);
}
...
Driver.prototype.pwmWrite = function(value, callback) {
  GPIO.write(this.pin, value);
  Cylon.Logger.debug(`Move servo with angle ${angle}`)
  Cylon.Logger.debug(GPIO.print())
  callback(null, value);
}
...
Driver.prototype.turnOff = function () {
  GPIO.high(this.pin);
  Cylon.Logger.debug(`Pin ${this.pin} just turned off`)
  Cylon.Logger.debug(GPIO.print())
}

Driver.prototype.turnOn = function () {
  GPIO.low(this.pin);
  Cylon.Logger.debug(`Pin ${this.pin} just turned on`)
  Cylon.Logger.debug(GPIO.print())
}
